...
combined_plot <- (p1 | p2 | p3) / 
  ...
  plot_annotation(
    title = 'Distribution of Average PV Across Subjects Among Singaporean Students'
  ) &
  theme(
...
# Function to create a boxplot for a given subject with the specified adjustments
create_boxplot <- function(data, subject_column, subject_name, show_y_labels) {
  p <- ggplot(data, aes(x = factor(ST004D01T), y = .data[[subject_column]], fill = factor(ST004D01T))) +
    geom_boxplot(outlier.color = "black") + # Change outliers to black
    labs(title = subject_name) + # Add title to each plot
    theme_minimal(base_size = 15) +
    theme(
      panel.grid.major = element_line(color = "grey", size = 0.1), # Make gridlines thinner
      panel.grid.minor = element_blank(),
      panel.background = element_blank(),
      axis.title.x = element_blank(),
      axis.text.x = element_blank(),
      axis.ticks.x = element_blank(),
      plot.title = element_text(hjust = 0.5) # Ensure title is centered
    )
  
  # Conditionally show the y-axis label
  if (!show_y_labels) {
    p <- p + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.title.y = element_blank())
  } else {
    p <- p + labs(y = 'Average Plausible Value')
  }
  
  # Add the legend for the first plot only
  if (subject_name == "Mathematics") {
    p <- p + theme(legend.position = "right") + 
      guides(fill=guide_legend(title="Gender")) # Add legend title
  } else {
...
# Define the max value for consistent y-axis scaling across boxplots
max_y_value <- max(Related_math_read_scie_data$avg_pvMATH, Related_math_read_scie_data$avg_pvREAD, Related_math_read_scie_data$avg_pvSCIE, na.rm = TRUE)

# Create boxplots
p1 <- create_boxplot(Related_math_read_scie_data, "avg_pvMATH", "Mathematics", TRUE) + 
  ylim(0, max_y_value) # Set consistent y-limits

p2 <- create_boxplot(Related_math_read_scie_data, "avg_pvREAD", "Reading", FALSE) + 
  ylim(0, max_y_value) # Set consistent y-limits

p3 <- create_boxplot(Related_math_read_scie_data, "avg_pvSCIE", "Science", FALSE) + 
  ylim(0, max_y_value) # Set consistent y-limits

# Combine the boxplots
combined_boxplots <- (p1 | p2 | p3) + 
